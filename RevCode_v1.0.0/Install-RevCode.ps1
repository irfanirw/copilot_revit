# ============================================================
#  RevCode v1.0.0 - Revit 2025 C# Code Editor  |  No-Admin Installer
#  Author: Irfan Irwanuddin
# ============================================================
#
#  Usage:   Right-click -> "Run with PowerShell"
#           or:  powershell -ExecutionPolicy Bypass -File Install-RevCode.ps1
#
#  To uninstall:  powershell -ExecutionPolicy Bypass -File Install-RevCode.ps1 -Uninstall
#
# ============================================================
...
$SourceDir      = Join-Path $ScriptDir "bin"

# ---- Helper ----
function Write-Banner {
    Write-Host ""
    Write-Host "  ======================================" -ForegroundColor Cyan
    Write-Host "    RevCode v$AppVersion"                  -ForegroundColor Cyan
    Write-Host "    C# Code Editor for Revit $RevitVersion" -ForegroundColor Cyan
    Write-Host "  ======================================" -ForegroundColor Cyan
    Write-Host ""
}
...
if (-not (Test-Path (Join-Path $SourceDir "$AppName.dll"))) {
    ...
    Write-Host ""
    Write-Host "  The 'bin' folder with compiled files is missing." -ForegroundColor Yellow
    Write-Host "  Please ensure this installer package is complete." -ForegroundColor Yellow
    Write-Host ""
    if (-not $Silent) { Read-Host "  Press Enter to exit" }
    exit 1
}
...
$sourceAddin = Join-Path $ScriptDir "$AppName.addin"
...
Write-Host "  ======================================" -ForegroundColor Green
Write-Host ""
Write-Host "  Launch Revit $RevitVersion to use $AppName." -ForegroundColor White
